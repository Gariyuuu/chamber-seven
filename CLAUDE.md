...
- **State management:** Component-local React state + one custom hook
  (`useGameRoom`) wrapping the WebSocket connection. **`zustand` `^5.0.14`
  is a listed dependency but is not imported anywhere in `src/` or
  `party/`** — confirmed via repo-wide search. Dead dependency; do not
  assume any global store exists.
...
│   ├── venues/                 # 6 PNG "tier" backdrop images — ⚠️ UNREFERENCED by any
│   │                            # source file (verified via repo-wide grep). Dead asset,
│   │                            # or an unfinished feature. See FEATURES.md / PROJECT_STATE.md.
│   ├── career-hero.png         # Used — Career Mode page hero banner
│   └── victory-burst.png       # ⚠️ UNREFERENCED by any source file. Same caveat as venues/.
...
1. **`public/venues/tier1.png`–`tier6.png` and `public/victory-burst.png`
   are unreferenced by any source file.** Severity: low (dead assets, no
   functional bug), but the v1.7 changelog entry
   (`src/lib/changelog.ts`) explicitly claims "6 mood-lit venue backdrops
   that escalate ... as you climb the ladder" — **this is not currently
   true of the shipped UI**; only a single static `career-hero.png` is
   shown on the Career Mode page regardless of level. Verified via
   repo-wide grep for `venue`/`tier1`/`victory` in `src/` and `party/` —
   zero non-comment matches. Either wire these images into
...
3. **Inconsistent local dev fallback host.** `useGameRoom.ts` defaults
   ...
   correct (matches `wrangler dev --port 8787`); `1999` appears to be a
   leftover from the original PartyKit CLI default port before the
   project migrated to `wrangler`/`partyserver` (per `.gitignore`'s
   `.partykit/` comment: "leftover local partykit dev state (migrated to
   wrangler)"). Low severity — `.env.local` always sets this var
   correctly in practice, but the dead/wrong fallback should eventually
   be fixed for consistency. Status: unresolved, not fixed this session
   (documentation-only audit).
4. **`zustand` is an unused dependency.** No import found anywhere in
   `src/` or `party/`. Low severity, pure technical debt (unnecessary
   install weight). Status: unresolved, not removed this session
   (removing a dependency is a code change outside this audit's scope —
   flagging per instructions rather than acting).
5. **README.md is stale** — still describes the game as strictly
   two-player, pre-dating the 2–4 player FFA, Career Mode, and (soon)
   team-mode expansions. This memory system supersedes it; update
   README.md when convenient.
...
16. Never remove a dependency without checking all usages first (note:
    `zustand` is flagged above as apparently unused — verify freshly with
    a repo-wide search before removing, don't just trust this file).
